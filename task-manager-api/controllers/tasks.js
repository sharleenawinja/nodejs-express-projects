...
const editTask = async (req, res) => {
  try {
    ...
    const task = await Task.findOneAndUpdate({ _id: taskId }, req.body, {
      new: true, //return the new task after updating
      runValidators: true, //run validators while updating
      // look into why overwrite does not work
      overwrite: true, //used with put to overwrite/remove/replace properties that are not passed in the request body
    });
